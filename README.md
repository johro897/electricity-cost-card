...
Electricity Cost Card is available directly in the HACS default catalog. Search for **Electricity Cost Card** under **Frontend** in HACS and install it — no custom repository needed.
...
- The price simulation slider now patches just the price-dependent DOM nodes (price value, badge, gauge fill, activity costs) directly during a drag, instead of fully re-rendering the card — a fast drag no longer triggers a burst of full re-renders, and the slider tracks the drag smoothly instead of only jumping on click
